...
const Rain: React.FunctionComponent<{
    direction: LiveWeatherData<DetailedRealtimeData["windDirDeg"]>;
    ...
    return (
        <Tooltip title={tooltip} variant="outlined">
            <Card
                className={`wind live-component ${
                    direction.val === null ? "no-data" : ""
                }`}
            >
                <Stack justifyContent={"center"} alignItems={"center"}>
                    <Typography textAlign={"left"} noWrap>
                        {direction.val !== null ? (
                            <BSArrowUpCircleFill style={style} />
                        ) : (
                            <SensorsOffIcon style={style} />
                        )}
                    </Typography>
                </Stack>
