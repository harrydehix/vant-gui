...
const WindSpeed: React.FunctionComponent<{
    pressure: LiveWeatherData<DetailedRealtimeData["press"]>;
    trend: LiveWeatherData<DetailedRealtimeData["pressTrend"]>;
    ...
    return (
        <Tooltip title={tooltip} variant="outlined">
            <Card
                className={`pressure live-component ${
                    pressure.val === null ? "no-data" : ""
                }`}
                style={{ width: "100%", height: "100%" }}
            >
                <Typography
                    level="h2"
                    startDecorator={trendIcon()}
                    textAlign={"left"}
                    noWrap
                >
                    ...
                    <Typography
                        ...
                        fontWeight={"100"}
                    >
                        {pressure.val === null ? "" : unit}
                    </Typography>
                </Typography>
                <Typography startDecorator={<CompressIcon />}>
                    {title} ({trend.str.toLowerCase()})
                </Typography>
            </Card>
        </Tooltip>
    );
